...
function formatHoursForText(store: NormalizedStore): string | undefined {
  if (!store.openingHours?.length) return undefined;
  return store.openingHours.map((period) => {
    if (period.closed) return `${period.day}は定休日`;
...
export function generateStoreContent(store: NormalizedStore): GeneratedStoreContent {
  ...
  const hoursText = formatHoursForText(store);
  const introParts = [`${store.name}は、${area}の${businessType}です。`, description].filter(Boolean);
  const features = unique([...store.services, ...store.menuItems, ...store.categories, store.areaName ? `${store.areaName}エリア` : "", hoursText ? "営業時間掲載" : ""]);
  ...
  const faqs = [
    ...
    hoursText ? { question: `${store.name}の営業時間は？`, answer: `営業時間は${hoursText}です。` } : undefined,
